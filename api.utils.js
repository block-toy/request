...
module.exports = {
  ...
  toUpper4:function(str) {
    return str.replace(/\/{(.*?)\}/g, ($0, $1) => $1[0].toUpperCase()+$1.substring(1))
  },
    ...
    if(!def.properties){
      ...
      Object.keys(def.properties).forEach(d=> {
        ...
        if(v.items){
          let tmp = ''
          if(v.items.$ref){
            tmp = v.items.$ref.replace('#/definitions/', '').replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g,'')
            if(v.type === 'array') tmp += '[]'
          }else{
          ...
          let tmp2 = ''
          tmp2 = v.$ref.replace('#/definitions/', '').replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g,'')
  ...
  coverType:function(v){
    switch(v.type){
      ...
      case 'array': return `${this.coverType(v.items)}[]`;
      default:
        return 'any'
    }
  }
