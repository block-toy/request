...
class SW{
  ...
  async getAllModules(){
    ...
    for (let index = 0; index < this.modules.length; index ++) {
      ...
      console.log('\x1B[36m%s\x1B[0m',`----------开始获取${mod.name}数据---------`)
      try {
        const res = await axios.get(this.config.host+mod.location)
        if(typeof res.data == 'string'){
          console.log('\x1B[31m%s\x1B[0m',`----------获取${mod.name}数据失败:${res.data}---------`)
        }else{
          // const fileContent = `
          ...
          this.generateApis(res.data.basePath, res.data.paths, res.data.basePath)
          console.log('\x1B[36m%s\x1B[0m',`----------获取${mod.name}数据成功---------`)
        }
      } catch (error) {
        console.log('\x1B[31m%s\x1B[0m',`----------获取${mod.name}数据失败---------`)
  ...
  generateMods(name,definitions){
    ...
    fs.writeFileSync(`src/types/${name}.d.ts`, `
    ...
    */
    ...
    finalPath.forEach((apiFn,index)=>{
      ...
      if(crt.responses[200].schema && crt.responses[200].schema.$ref){
        const schema = crt.responses[200].schema.$ref.replace('#/definitions/', '').replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g,'')
        ResInterface += `interface ${resObj} extends ${name}.${schema} {[k:string]:any} \n`
      }else{
        ResInterface += `interface ${resObj} { [k:string]:any }`
      }
      const urlPath = '`'+`${rootPath}${crt.api?.replace(/\{(.*?)\}/g, ($0, $1) => '${'+`data.${$1}`+'}')}` +'`'
      /** 处理入参interface */
      let schemaName = []
      if(crt.parameters){
        let paramStr = ''
        crt.parameters.forEach(p=>{
          ...
          if(p.schema && p.schema.$ref){
            const schema = p.schema.$ref.replace('#/definitions/', '').replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g,'')
        ...
        fnString +=`
          /** 
            ...
            * summary: ${crt.summary}
          */
          export const ${apiFnName} = (data:${reqObj}${params}) => request<${resObj},${resObj}['data']>(${urlPath},'${crt.method}', data)
          \n
        ...
        fnString +=`
          /** 
            ...
            * summary: ${crt.summary}
          */
          export const ${apiFnName} = () => request<${resObj},${resObj}['data']>(${urlPath},'${crt.method}')
          \n
          ${name}.${apiFnName} = ${apiFnName}
        `
        }
    ...
    fs.writeFileSync(`src/apis/${name}.ts`, `
      import ${this.config.isDefaultExport ? 'request' : '{ request }' } from '${this.config.requestPath}'
      const ${name}: any = {};
      ${fnString}
      ${ReqInterface}
      ${ResInterface}
